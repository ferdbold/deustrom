        ...
        public void RetractHooksMassModification(float retractionForceToAdd) {
            if (!_endingHookIsSet) {
                if (beginningHook.currentAnchorPoint.GetIslandChunk().parentIsland != null
                    && beginningHook.currentAnchorPoint.GetIslandChunk().parentIsland.chunks.Count > 3) {
                    thrower.rigidbody.AddForce((beginningHook.transform.position - thrower.transform.position) * retractionForceToAdd, ForceMode2D.Impulse);
                } else {
